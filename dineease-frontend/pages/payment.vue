...
import { useWebSocket } from '@/lib/websocket'; // Import the WebSocket utility
...
console.log(userId)
const { notification, connectWebSocket, closeWebSocket } = useWebSocket(userId.value);

// Convert notification string to an array for cleaner display
const notificationMessages = computed(() => notification.value.split('\n').filter(Boolean));
