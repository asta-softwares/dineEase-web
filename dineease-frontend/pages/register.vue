<template>
      ...
      <Card class="w-[500px]">
        ...
        <CardContent class="grid gap-4">
          <div class="grid gap-2">
            ...
            <Input id="name" v-model="name" placeholder="Full Name" />
          </div>
          <div class="grid gap-2">
  ...
  const canSubmit = computed(() => name.value && email.value && phone.value && password.value && !passwordMismatch.value && passwordStrength.value >= 2)
  ...
  const handleRegister = async () => {
    try {
      ...
      await register(email.value, phone.value, username.value, password.value, name.value, "restaurant_owner")
